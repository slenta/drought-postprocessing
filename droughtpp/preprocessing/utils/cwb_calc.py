...
def compute_cwb_global(
    ...
    # Normalize time coordinates to a proper monthly index.
    temp = temp.assign_coords(
        time=pd.to_datetime(temp["time"].values)
        .to_period("M")
    ...
    # Set time encoding for proper NetCDF interpretation
...
if __name__ == "__main__":
    ...
    out_path = f"/work/bk1318/k202208/crai/hindcast-pp/data/spei/cwb/"
    ...
    # Define output file
    hc_output_file = out_path + f"dwd-hindcast_cwb_ly{lead_year}.nc"
    ref_output_file = out_path + f"era5-tamsat_cwb_ly.nc"
