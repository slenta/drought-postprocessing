...
def init_enc_conv_configs(
    conv_factor, img_size, enc_dec_layers, pool_layers, start_channels
):
    conv_configs = []
    for i in range(enc_dec_layers):
        ...
        conv_config["img_size"] = [
            size // (2**i) if size % (2**i) == 0 else size // (2**i) + 1
            for size in img_size
        ]
        conv_config["rec_size"] = [
            size // 2 if size % 2 == 0 else size // 2 + 1
            for size in conv_config["img_size"]
        ]
    ...
    for i in range(pool_layers):
        ...
        conv_config["img_size"] = [
            (
                size // (2 ** (enc_dec_layers + i))
                if size % (2 ** (enc_dec_layers + i)) == 0
                else size // (2 ** (enc_dec_layers + i)) + 1
            )
            for size in img_size
        ]
        conv_config["rec_size"] = [
...
def init_dec_conv_configs(
    conv_factor, img_size, enc_dec_layers, pool_layers, start_channels, end_channels
):
    conv_configs = []
    for i in range(pool_layers):
        conv_config = {}
        ...
        conv_config["img_size"] = [
            (
                size // (2 ** (enc_dec_layers + pool_layers - i - 1))
                if size % (2 ** (enc_dec_layers + pool_layers - i - 1)) == 0
                else size // (2 ** (enc_dec_layers + pool_layers - i - 1)) + 1
            )
            for size in img_size
        ]
        conv_config["rec_size"] = [
